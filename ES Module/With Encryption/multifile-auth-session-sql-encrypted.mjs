...
function getDbPaths(folder, name) {
  const base = path.join(folder, `${encodeFileName(name)}.db`);
  return {
    dbPath: base,
    bakPath: `${base}.bak`,
    tmpPath: `${base}.tmp`,
...
async function persistDatabase(db, dbPath, bakPath, tmpPath) {
  ...
  try {
    ...
    await fsp.unlink(tmpPath).catch(() => {});
    throw new Error(`Failed to persist database: ${dbPath}`);
  }
}

async function readValueFromFile(SQL, dbPath, bakPath, tmpPath) {
  ...
  try {
    ...
    const raw =
      getRow(db, 'data') ??
      getRow(db, 'value') ??
      getRow(db, 'creds'); // backward compatibility
...
async function writeValueToFile(SQL, dbPath, bakPath, tmpPath, value) {
  let db;
  try {
    const bytes = await openBestDatabase(SQL, dbPath, bakPath, tmpPath);
    ...
    setRow(db, 'data', serializeValue(value));
...
async function deleteValueFile(dbPath, bakPath, tmpPath) {
  await Promise.all([
...
export async function useSqlAuthState(folder = './session') {
  ...
  const credsPaths = getDbPaths(folder, 'creds');

  const readKeyFile = async (type, id) => {
    const name = `key:${type}:${id}`;
    const { dbPath, bakPath, tmpPath } = getDbPaths(folder, name);
    return readValueFromFile(SQL, dbPath, bakPath, tmpPath);
  };

  const writeKeyFile = async (type, id, value) => {
    const name = `key:${type}:${id}`;
    const { dbPath, bakPath, tmpPath } = getDbPaths(folder, name);
    await writeValueToFile(SQL, dbPath, bakPath, tmpPath, value);
  };

  const removeKeyFile = async (type, id) => {
    const name = `key:${type}:${id}`;
    const { dbPath, bakPath, tmpPath } = getDbPaths(folder, name);
    await deleteValueFile(dbPath, bakPath, tmpPath);
  };

  let creds = await readValueFromFile(SQL, credsPaths.dbPath, credsPaths.bakPath, credsPaths.tmpPath);

  if (!creds) {
    creds = initAuthCreds();
    await writeValueToFile(SQL, credsPaths.dbPath, credsPaths.bakPath, credsPaths.tmpPath, creds);
  }
  ...
  const state = {
    ...
    keys: {
      get: async (type, ids) => {
        ...
        for (const id of ids) {
          let value = await readKeyFile(type, id);
        ...
        await lock.runExclusive(async () => {
          for (const category in data) {
            for (const id in data[category]) {
              ...
              if (value == null) {
                await removeKeyFile(category, id);
              } else {
                await writeKeyFile(category, id, value);
              }
            }
          }
        });
  ...
  const close = async () => {
    await lock.runExclusive(async () => {
      // Nothing kept open globally, but we keep this for API compatibility.
    });
  };
